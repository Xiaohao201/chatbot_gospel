...
async def query_content_by_content_id(content_id: str) -> Dict:
    ...
    sql: str = "select * from bilibili_video where video_id = %s"
    params = (content_id,)

    if isinstance(async_db_conn, AsyncSqliteDB):
        sql = "select * from bilibili_video where video_id = ?"

    rows: List[Dict] = await async_db_conn.query(sql, params)
    if len(rows) > 0:
        return rows[0]
    return dict()
...
async def query_comment_by_comment_id(comment_id: str) -> Dict:
    ...
    """
    async_db_conn: Union[AsyncMysqlDB, AsyncSqliteDB] = media_crawler_db_var.get()
    sql: str = "select * from bilibili_video_comment where comment_id = %s"
    params = (comment_id,)

    if isinstance(async_db_conn, AsyncSqliteDB):
        sql = "select * from bilibili_video_comment where comment_id = ?"
...
async def query_creator_by_creator_id(creator_id: str) -> Dict:
    ...
    """
    async_db_conn: Union[AsyncMysqlDB, AsyncSqliteDB] = media_crawler_db_var.get()
    sql: str = "select * from bilibili_up_info where user_id = %s"
    params = (creator_id,)

    if isinstance(async_db_conn, AsyncSqliteDB):
        sql = "select * from bilibili_up_info where user_id = ?"
...
async def query_contact_by_up_and_fan(up_id: str, fan_id: str) -> Dict:
    ...
    """
    async_db_conn: Union[AsyncMysqlDB, AsyncSqliteDB] = media_crawler_db_var.get()
    sql: str = "select * from bilibili_contact_info where up_id = %s and fan_id = %s"
    params = (up_id, fan_id)

    if isinstance(async_db_conn, AsyncSqliteDB):
        sql = "select * from bilibili_contact_info where up_id = ? and fan_id = ?"
...
async def query_dynamic_by_dynamic_id(dynamic_id: str) -> Dict:
    ...
    """
    async_db_conn: Union[AsyncMysqlDB, AsyncSqliteDB] = media_crawler_db_var.get()
    sql: str = "select * from bilibili_up_dynamic where dynamic_id = %s"
    params = (dynamic_id,)

    if isinstance(async_db_conn, AsyncSqliteDB):
        sql = "select * from bilibili_up_dynamic where dynamic_id = ?"
