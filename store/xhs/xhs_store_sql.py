...
async def query_content_by_content_id(content_id: str) -> Dict:
    ...
    sql: str = "select * from xhs_note where note_id = %s"
    params = (content_id,)

    if isinstance(async_db_conn, AsyncSqliteDB):
        sql = "select * from xhs_note where note_id = ?"
...
async def query_comment_by_comment_id(comment_id: str) -> Dict:
    ...
    """
    async_db_conn: Union[AsyncMysqlDB, AsyncSqliteDB] = media_crawler_db_var.get()
    sql: str = "select * from xhs_note_comment where comment_id = %s"
    params = (comment_id,)

    if isinstance(async_db_conn, AsyncSqliteDB):
        sql = "select * from xhs_note_comment where comment_id = ?"
...
async def query_creator_by_user_id(user_id: str) -> Dict:
    ...
    """
    async_db_conn: Union[AsyncMysqlDB, AsyncSqliteDB] = media_crawler_db_var.get()
    sql: str = "select * from xhs_creator where user_id = %s"
    params = (user_id,)

    if isinstance(async_db_conn, AsyncSqliteDB):
        sql = "select * from xhs_creator where user_id = ?"
