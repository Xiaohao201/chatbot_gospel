...
from async_db import AsyncMysqlDB, AsyncSqliteDB
...
async def init_db(db_type: str = None):
    # 如果没有指定 db_type，使用配置中的设置
    if db_type is None:
        db_type = base_config.SAVE_DATA_OPTION

    # 初始化表结构
    await init_table_schema(db_type)

    # 根据数据库类型创建连接实例并设置到 ContextVar
    if db_type == "db":
        # MySQL
        db_conn = AsyncMysqlDB()
    elif db_type == "sqlite":
        # SQLite
        from config.db_config import sqlite_db_config
        db_conn = AsyncSqliteDB(db_path=sqlite_db_config['db_path'])
    else:
        # 其他类型（如 json、csv）不需要数据库连接
        db_conn = None

    # 设置到 ContextVar 中，供其他模块使用
    if db_conn is not None:
        media_crawler_db_var.set(db_conn)
        utils.logger.info(f"[init_db] Database connection initialized for {db_type}")
